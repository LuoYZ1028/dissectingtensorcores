...
__global__ void tensr1688_flops(uint64_t *startClk, uint64_t *stopClk, half *a, half *b, float *res,
  ...
  b = b + warpid * 16*16; // n*k = 16*16
  res = res + warpid * 16*16;// m*n = 16*16
  ...
  float frag_D[8 * ILPconfig]; // result(fp32) 8 f32 registers
  ...
  float *D = C;  // D = A*B + C. thus point *C and *D to same address 
  ...
  for (int j = 0; j < ITERS; ++j) {
    asm volatile(
        "wmma.mma.sync.aligned.m8n32k16.row.col.f32.f16.f16.f32 "
...
float tensor1688_max_flops(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    unsigned total_A_SIZE =
        16*16 * (TOTAL_THREADS / WARP_SIZE) * ILPconfig; // asume one 16x16 matrix per warp
    unsigned total_B_SIZE =
        16*16 * (TOTAL_THREADS / WARP_SIZE) * ILPconfig; // asume one 16*16 matrix per warp
    ...
    //           (float)total_time;
    fma_bw = ((float)(ITERS * 16 * 16 * 16 * ILPconfig * //0 *
    ...
    std::cout << "wmma.mma.sync.aligned.m16n16k16.row.col.f32.f16.f16.f32  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    ...
    std::cout<<"***********************************"<<std::endl;
    std::cout << "wmma.mma.sync.aligned.m16n16k16.row.col.f32.f16.f16.f32 microbenchmark with ILP = " << ILPconfig << std::endl;
    for(auto& e:warps){
