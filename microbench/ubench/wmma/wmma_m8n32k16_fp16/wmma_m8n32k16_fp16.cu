...
__global__ void tensor83216_flops(uint64_t *startClk, uint64_t *stopClk, 
...
template <class T, class R> 
float tensor83216_max_flops(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
	...
	// 给 mma 操作计时
	tensor83216_flops<T, R><<<BLOCKS_NUM, THREADS_PER_BLOCK>>>(
...
int main() {
	...
	for (auto& e:warps) {
		...
		tensor83216_max_flops<__half, float>(32 * e); // 每个 warp 有32个线程
		std::cout << std::endl;
	}
	return 0;
}
